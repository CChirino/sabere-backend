...
const student = ref<{ name: string } | null>(null);
...
const fetchSchedule = async () => {
    loading.value = true;
    try {
        const response = await fetch(`/api/v1/guardian/student/${props.studentId}/schedule`, {
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest',
            },
...
onMounted(async () => {
    await Promise.all([fetchStudentInfo(), fetchSchedule()]);
});
</script>

<template>
    <Head :title="`Horario - ${student?.name || 'Estudiante'}`" />
    ...
    <AppLayout>
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Horario de Clases</h1>
        ...
        <div v-if="!loading && section && !hasAnySchedule" class="mb-6 rounded-lg bg-amber-50 border border-amber-200 p-4">
            <div class="flex items-start">
                ...
                <div>
                    ...
                    <p class="text-sm text-amber-600">El horario de esta sección aún no ha sido configurado por la administración.</p>
                </div>
            </div>
        </div>
