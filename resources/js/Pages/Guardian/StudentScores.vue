...
const fetchGradedTasks = async () => {
    try {
        const response = await fetch(`/api/v1/guardian/student/${props.studentId}/scores`, {
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest',
            },
...
onMounted(async () => {
    await Promise.all([fetchStudentInfo(), fetchGradedTasks()]);
    loading.value = false;
});
</script>

<template>
    <Head :title="`Boleta - ${student?.name || 'Estudiante'}`" />
    ...
    <AppLayout>
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Boleta de Calificaciones</h1>
        ...
        <template v-else>
            ...
            <div v-if="!hasGrades" class="text-center py-12">
                ...
                <p class="mt-1 text-gray-500">Aún no hay calificaciones registradas.</p>
            </div>
                    ...
                    <template #header>
                        <div class="flex items-center justify-between">
                            ...
                            <div class="text-right">
                                ...
                                <Badge :color="getScoreColor(parseFloat(String(getSubjectAverage(tasks))), 20)" class="ml-2">
